        ...
        public double CalculateFitness(int[,] schedule, List<int[]> employeePreferences, ScheduleParameters scheduleParameters)
{
    ...
    for (int worker = 0; worker < numberOfWorkers; worker++)
    {
        ...
        if (preferences.All(p => p == -1)) continue; // Jeśli wszystkie preferencje to -1, pomijamy tego pracownika
        ...
        for (int day = 0; day < daysInWeek; day++)
        {
            ...
            {
                fitness -= 3; // Kara za przypisanie zmiany, kiedy preferencja to dzień wolny
            }
            else if (preferredShift != 0 && assignedShift == 0)
            {
                fitness -= 3; // Kara za przypisanie dnia wolnego, kiedy preferencja to zmiana
            }
        }
    }
